...
def transforms_for_noise(inputs_u2, std):
    ...
    result = inputs_u2 + gaussian

    return result

def transforms_for_noise_3d(inputs_u2, std):

    gaussian = np.random.normal(0, std, (inputs_u2.shape[0], inputs_u2.shape[1], inputs_u2.shape[2], inputs_u2.shape[3],inputs_u2.shape[4]))
    gaussian = torch.from_numpy(gaussian).float().cuda()
    result = inputs_u2 + gaussian
...
def transforms_for_lightness(ema_inputs):
    MAX_VALUE=100
    lightness= np.random.randint(-50, 50, ema_inputs.shape[0])

    for idx in range(ema_inputs.shape[0]):
        img = np.transpose(ema_inputs[idx].cpu().numpy(), (1, 2, 0)).astype(np.float32) / 255.0

        # 颜色空间转换 BGR转为HLS
        hlsImg = cv2.cvtColor(img, cv2.COLOR_BGR2HLS)

        # 1.调整亮度（线性变换)
        hlsImg[:, :, 1] = (1.0 + lightness[idx] / float(MAX_VALUE)) * hlsImg[:, :, 1]
        ...
        ema_inputs[idx] = torch.from_numpy(lsImg.transpose((2, 0, 1))).cuda()

    return ema_inputs

def transforms_for_down_scale(ema_inputs,model=1):
    size=ema_inputs.shape[2]
    scale_mask =np.random.uniform(low=0.8, high=0.97, size=ema_inputs.shape[0])
    scale_mask = scale_mask * size
    scale_mask = [int(item) for item in scale_mask]

    for idx in range(ema_inputs.shape[0]):
        img = np.transpose(ema_inputs[idx].cpu().numpy(), (1, 2, 0)).astype(np.uint8)
        if model == 1: #从左上角截取
            img=img[0:scale_mask[idx],0:scale_mask[idx],:]
        else:#从右下角开始截取
            img = img[ema_inputs.shape[2]-scale_mask[idx]:ema_inputs.shape[2], ema_inputs.shape[2]-scale_mask[idx]:ema_inputs.shape[2],:]
        # resize
        resized_img = cv2.resize(img, (ema_inputs.shape[2], ema_inputs.shape[2]), interpolation=cv2.INTER_LINEAR)
        # to tensor
        ema_inputs[idx] = torch.from_numpy(resized_img.transpose((2, 0, 1))).cuda()

    return ema_inputs

def transforms_for_3ddownscale(ema_inputs,model=1):
    size=ema_inputs.shape[2]
    scale_mask =np.random.uniform(low=0.8, high=0.97, size=ema_inputs.shape[0])
    scale_mask = scale_mask * size
    scale_mask = [int(item) for item in scale_mask]
...
if __name__ == '__main__':
    ...
    exit(0)
    plt.figure(3)
    plt.imshow(ema_inputs[2])
    plt.figure(4)
    plt.imshow(ema_inputs[3])
    plt.figure(5)
    plt.imshow(ema_inputs[4])
    plt.figure(6)
    plt.imshow(ema_inputs[5])
    plt.figure(7)
    plt.imshow(ema_inputs[6])
    plt.figure(8)
    plt.imshow(ema_inputs[7])
    plt.axis('off')
    plt.show()
